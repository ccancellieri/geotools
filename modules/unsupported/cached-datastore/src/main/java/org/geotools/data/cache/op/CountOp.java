...
public class CountOp extends BaseFeatureSourceOp<Integer> {
	...
	public CountOp(final CacheManager cacheManager, final String uid)
			throws IOException {
		...
		final SimpleFeatureSource featureSource = new DelegateContentFeatureSource(
			...
			protected FeatureReader<SimpleFeatureType, SimpleFeature> getReaderInternal(
				...
				if (isCached(query) && !isDirty(query)) {
					return cacheManager.getCache().getFeatureReader(query,
							transaction);
				} else {
					return new PipelinedContentFeatureReader(getEntry(),
							sourceQuery, cacheQuery, cacheManager, transaction);
				}
			}
		};
