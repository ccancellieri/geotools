...
public class STRCachedFeatureSource extends BaseCachedFeatureSource {
    ...
    public int getCountInternal(Query query) throws IOException {
        try {
            ...
            throw new DataSourceException("Error occurred creating new spatial index", e);
        } finally {
    ...
    @Override
    protected int count(Query origQuery, Query integratedQuery) {
        return index.query((Envelope) getEnvelope(origQuery.getFilter())).size();
    }
    ...
    @Override
    protected FeatureReader query(Query origQuery, Query integratedQuery) throws IOException {

        updateIndex(integratedQuery);
        
        final List features;
        try {
            lockIndex.readLock().lock();
            features = index.query((Envelope) getEnvelope(origQuery.getFilter()));
//            return new FilteringFeatureReader<SimpleFeatureType, Feature>(new SimpleListReader(features, getSchema()),origQuery.getFilter());
            // query.getFilter());
        } finally {
