 ...
 * 
 */
abstract class ImageMosaicDatastoreWalker extends ImageMosaicWalker {
    ...
    public void run() {
        ...
        try {
            ...
            try {
                ...
                for (String typeName : configHandler.getCatalog().getTypeNames()) {
                    
                    final Query query = new Query(typeName);
                    final SimpleFeatureCollection coll = configHandler.getCatalog().getGranules(
                            query);
//                    TODO we might want to remove this in the future for performance
                    numFiles=coll.size();
                    
                    final SimpleFeatureIterator it = coll.features();
                    
                    // TODO setup index name
                    
                   
                    // final MosaicConfigurationBean config= configurations.get(typeName);
                    while (it.hasNext()) {
                        SimpleFeature feature = it.next();
                        // String locationAttrName=config.getCatalogConfigurationBean().getLocationAttribute();
                        ...
                        if (locationAttrObj instanceof String) {
                            if (configHandler.getRunConfiguration().isAbsolute()) {
                                indexing = new File((String) locationAttrObj);
                            } else {
                                indexing = new File(configHandler.getRunConfiguration()
                                        .getRootMosaicDirectory(), (String) locationAttrObj);
                            }
